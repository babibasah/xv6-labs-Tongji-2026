...
_vmprint(pagetable_t pagetable, int level, uint64 base_va)
{
  for(int i = 0; i < 512; i++){
    ...
    if(pte & PTE_V){
      uint64 child_va = base_va | ((uint64)i << (12 + 9 * level));
      ...
      if((pte & (PTE_R | PTE_W | PTE_X)) == 0){
        _vmprint((pagetable_t)pa, level - 1, child_va);
      }
    }
  }
}
...
int supermappages(pagetable_t pagetable, uint64 va, uint64 size, uint64 pa, int perm)
{
  ...
  if((va % SUPERPGSIZE) != 0 || (pa % SUPERPGSIZE) != 0 || (size % SUPERPGSIZE) != 0)
    panic("supermappages: alignment");
...
{
  pte_t *pte1 = walk_super(pagetable, va, 0);
  if(!pte1 || !(*pte1 & PTE_V) || !(*pte1 & (PTE_R | PTE_W | PTE_X)))
    return;

  pagetable_t pt0 = (pagetable_t)kalloc();
  if(pt0 == 0)
    panic("demote_superpage: kalloc failed");

  memset(pt0, 0, PGSIZE);

  uint64 pa = PTE2PA(*pte1);
  int flags = PTE_FLAGS(*pte1);

  for(int i = 0; i < 512; i++) {
    pt0[i] = PA2PTE(pa + i * PGSIZE) | flags;
  }

  *pte1 = PA2PTE(pt0) | PTE_V;
}
...
uvmunmap(pagetable_t pagetable, uint64 va, uint64 npages, int do_free)
{
  ...
  for(a = va; a < end; ){
    pte_t *pte1 = walk_super(pagetable, a, 0);
    if(pte1 && (*pte1 & PTE_V) && (*pte1 & (PTE_R | PTE_W | PTE_X))){
